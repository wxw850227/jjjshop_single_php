					...
					<el-table-column label="商品" width="400px">
						<template #default="scope">
							<div class="product-info">
								<div class="pic"><img v-img-url="scope.row && scope.row.product.image[0].file_path"
										alt="" /></div>
								<div class="info">
									<div class="name">{{scope.row && scope.row.product.product_name}}</div>
									<div class="price">￥{{scope.row && scope.row.product.product_price}}</div>
									<div class="name">订单号:{{scope.row && scope.row.orderM?scope.row.orderM.order_no:''}}
									</div>
								</div>
							</div>
						</template>

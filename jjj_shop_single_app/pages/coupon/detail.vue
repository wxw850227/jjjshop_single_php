<template>
	<view class="coupon_bg" v-if="!loading">
		...
		<view class="coupon-detail-top">
			...
			<!--  -->
			<view class="redF6 f26 fb pt30 d-c-c" v-if="apply_range!=10">
			...
			<view class="o-h pro_list" v-if="apply_range!=10">
				<view class="d-b-c f-w">
					<view class="pro_item" v-for="(item,index) in listData" :key='index'
						...
						<view class="fb redF6 tc"><text class="f22">￥</text><text class="f32">{{item.product_price}}</text>
						</view>
						<view class="d-c-c">
		...
		methods: {
			...
			getData() {
				...
				if (self.apply_range != 20) {
					...
					params.list_rows = self.list_rows;
				}
